function [w_union,w_inter,G_union,G_inter,w_raw] = mblasso(X,inopts)
% Meinshausen-Buhlmann neighborhood selection approach 
...
% lambda vector from max lambda to min lambda for warm start
...
for i=1:p
    ...
    % Lasso variant over entire path using M. Schmidt's code
    tempBeta = lassoSchmidt(currX,Yvec,lamVec);
